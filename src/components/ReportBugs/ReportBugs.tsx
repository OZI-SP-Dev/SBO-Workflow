...
export const ReportBugs: FunctionComponent = () => {
  ...
  const submitReport = async () => {
    ...
    if (report) {
      ...
      const body = `${report}<br><br><hr><br>Message sent by ${
        from.Title
      }<br>On ${DateTime.local().toLocaleString(
        DateTime.DATETIME_MED
      )}<br>From route ${location.pathname}`;
      await email.sendEmail(to, subject, body, undefined);
      clearForm();
    }
  };
  ...
  return (
    <>
      ...
      >
        <Form>
          ...
          <Form.Control.Feedback type="invalid">
            {submitAttempted && !report
              ? "You must provide details on the bug!"
              : ""}
      ...
      <Nav.Link
        ...
        disabled={contactUsContext.loading}
      >
