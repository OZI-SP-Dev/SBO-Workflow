...
export const StatusWorkflow: FunctionComponent<IStatusWorkflowProps> = (
  props
) => {
  return (
    <Card className={"sbo-gray-gradiant " + props.className}>
      <Col className="m-3">
        <h4>
          {props.process.ProcessType} Process:{" "}
          {props.process.SolicitationNumber}
        </h4>
        <p className="mb-0">
          <strong>Current SBAT Stage: </strong>
          {props.process.CurrentStage}
          <br />
          <strong>Next SBAT Stage: </strong>
          {nextStageText(props.process)}
        </p>
