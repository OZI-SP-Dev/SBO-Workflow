...
export function isIPerson(person: any): person is IPerson {
  return (
    (person as IPerson).Id !== undefined &&
...
export const getCurrency = (value: string | number): string => {
  ...
  let dollars: string = numbers.substring(
    0,
  ...
  if (dollars.length > 3) {
    ...
    while (commaIndex > 0) {
      dollars =
        dollars.substring(0, commaIndex) + "," + dollars.substring(commaIndex);
      commaIndex -= 3;
    }
  }
  let cents: string =
    periodIndex > -1 ? numbers.substring(periodIndex + 1) : "00";
  if (cents.length > 2) {
