...
export function usePagedProcesses(): IPagedProcesses {
  ...
  const fetchProcessesPage = async (refreshCache?: boolean) => {
    try {
      ...
      while (
        processesCopy.length < filters.page &&
        processesCopy[processesCopy.length - 1].hasNext
      ) {
        processesCopy.push(
          await processesCopy[processesCopy.length - 1].getNext()
        );
      }
  ...
  const addFilter = (
    ...
    if (filterValue) {
      ...
      let oldFilterIndex = newFilters.findIndex(
  ...
  return {
    processes:
      processes.length >= filters.page
        ? processes[filters.page - 1].results
        : [],
    activeFilters: filters.fieldFilters.map((filter) => filter.fieldName),
    page: filters.page,
    hasNext:
      processes.length >= filters.page
        ? processes[filters.page - 1].hasNext
        : false,
    loading,
    fetchCachedProcess,
    refreshPage,
