...
export function useEmail(): IEmailSender {
  ...
  const sendSubmitEmail = async (process: IProcess): Promise<void> => {
    ...
    let body = `Procurement RFP # ${
      process.SolicitationNumber
    } has been assigned to you for Buyer Review on ${process.Modified.toLocaleString(
      DateTime.DATETIME_MED
        ...
        Link to procurement: ${
          emailApi.siteUrl
        }/app/index.aspx?route=%23%2FProcesses%2FView%2F${process.Id}
  ...
  const sendAdvanceStageEmail = async (
    ...
    } on ${process.CurrentStageStartDate.toLocaleString(DateTime.DATETIME_MED)}.
  ...
  const sendRejectStageEmail = async (
    ...
    let body = `Procurement RFP # ${
      process.SolicitationNumber
    } has been returned to you for rework of ${
      process.CurrentStage
    } on ${process.CurrentStageStartDate.toLocaleString(DateTime.DATETIME_MED)}.
